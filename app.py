...
with st.sidebar:
    st.markdown("""
    <div style="padding:1.2rem 1rem 0.6rem;border-bottom:0.5px solid #ebebeb">
      <div style="font-size:10px;font-weight:600;color:#bbb;letter-spacing:0.08em;
                  text-transform:uppercase">电子结构理论</div>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("""
    <div style="font-size:10px;font-weight:600;color:#bbb;letter-spacing:0.08em;
                text-transform:uppercase;padding:0.75rem 1rem 0.3rem">学习模块</div>
    """, unsafe_allow_html=True)

    for m in MODULES:
        col_btn, col_tag = st.columns([5, 1])
        with col_btn:
            if st.button(f"  {m['title']}", key=f"home_mod_{m['key']}",
                         use_container_width=True):
                st.switch_page(m["path"])
        with col_tag:
            st.markdown(f"""
            <div style="height:36px;display:flex;align-items:center;justify-content:center">
              <span style="font-size:9px;padding:2px 5px;border-radius:6px;
                           font-weight:600;background:{m['tag_bg']};
                           color:{m['tag_col']};white-space:nowrap">{m['tag']}</span>
            </div>
            """, unsafe_allow_html=True)

    st.markdown('<div style="border-top:0.5px solid #ebebeb;padding:0.75rem 0.75rem 0;margin-top:0.5rem"></div>',
                unsafe_allow_html=True)
    if st.button("引言 →", key="home_start", use_container_width=True, type="primary"):
...
<div class="hero-screen">
  <div class="hero-left">
    ...
    <div class="stats">
        <div class="stat"><div class="stat-n" style="color:#534AB7">7</div><div class="stat-l">学习模块</div></div>
